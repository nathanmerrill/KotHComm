...
import com.ppcg.kothcomm.utils.iterables.Itertools;
...
public class MamAggregator<T> implements Aggregator<Ranking<T>> {
    ...
    @Override
    public Ranking<T> aggregate(MutableList<Ranking<T>> scores) {
        MutableList<MutableSortedSet<T>> votes = scores.collect(Ranking::rank);
        ...
        return Ranking.toRanking(players.toSortedList((o1, o2) -> {
    ...
    private int getPreferenceCount(Twin<T> players, MutableList<Ranking<T>> votes) {
